...
from loader import Llama_1K, NetBatchSampler, MAX_NODES_BATCH
from .graph import Graph_LLM, GraphBatch
from .utils import log
from .ddp_utils import is_ddp
from .ops import NetworkLight

print("################## Load Llama Dataset ##################")
class Llama_1KDDP(Llama_1K):
    r"""
    ...
    def __init__(self,
        ...
        if 'nets_dir' in kwargs and kwargs['nets_dir'] != './data':
            ...
            deepnets_file = 'Llama1K_%s.pkl' % (split if split in ['train', 'search'] else 'eval')
            h5_file = os.path.join(nets_dir, deepnets_file)
            if not os.path.exists(h5_file):
                kwargs['nets_dir'] = './data'
            log('Llama1KDDP nets_dir set to %s as Llama1K files not found at %s' % (kwargs['nets_dir'], nets_dir))

        super(Llama_1KDDP, self).__init__(**kwargs)
    ...
    def loader(meta_batch_size=1, dense=True, **kwargs):
        nets = Llama_1KDDP(dense=dense, **kwargs)
