...
class Trainer:
    ...
    def update(self, images, targets, models = None, graphs=None, embeddings=None, param_groups_map=None, ghn=None):
        ...
        try:
                ...
                if self._is_ghn:
                    ...
                    if hasattr(graphs, 'nets') and len(graphs.nets) > 0:
                        ...
                        for nets_args in graphs.net_args:
                            ...
                            # nets_args['num_layers'] = layers
                            # nets_args['hidden_dim'] = hidden_dim
                            # nets_args['mlp_dim'] = mlp_dim
                            ...
                            #     patch_size = 2, ## cifar10: patchs-size=2
                            #     num_layers = layers,
                            #     num_heads = heads,
                            ...
                            net = _vision_transformer(
                                patch_size = 2, ## cifar10: patchs-size=2; imagenet: 16
                                num_layers = nets_args['num_layers'],
                                num_heads = nets_args['num_heads'],
                                hidden_dim = nets_args['hidden_dim'],
                                mlp_dim = nets_args['mlp_dim'],
                                num_classes = 100, # cifar: 100, imagenet: 1000
                                image_size = 32,  # cifar: 32, imagenet: 224
                                weights = None,
                                progress = False,
                                ).to(self.main_device)
            ...
            if self._is_latent:
                self.metrics['gradnorm'].update((avg_ddp_metric(total_norm_clip)
                                                 if self.ddp else total_norm_clip).item(), 1)
            self._step += 1
